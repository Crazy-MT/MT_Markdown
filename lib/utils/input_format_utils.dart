...
class NumberInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (numberLength != 0) {
      if (newValue.text.length > numberLength &&
    ...
    if (newValue.text.contains(".") &&
        newValue.text.split(".")[1].length > decimalLength) {
      return oldValue;
    }
...
class MaxInputFormatter extends TextInputFormatter {
  ...
  MaxInputFormatter(this.maxInputLength);
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (newValue.text.length > maxInputLength) {
      Utils.showToastMsg("不可超过$maxInputLength字哦～");
      return oldValue;
    }
    if (newValue.text.contains("   ")) {
      return oldValue;
